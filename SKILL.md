...
## 文档索引
先读 `AGENTS.md`。Windows 开发读 `doc/02-架构与原理.md`；排障读 `doc/05-已知问题与踩坑记录.md`；进度读 `doc/04-项目进度.md`；技术方案读 `doc/03-技术方案.md`；Mac 开发读 `doc/06-Mac版方案.md`。
...
## 当前状态与版本
Windows v1.9.14 已部署用户常驻目录，尚未发 GitHub Release；版本字段在 `airpods_buddy.ahk` 的 `APP_VERSION`。
