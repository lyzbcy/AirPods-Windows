...
$f=Fake @((Ep 'render' 0 1),(Ep 'capture' 1 8));$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$false,1)
...
$f=Fake @((Ep 'render' 0 1));$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$false,0)
Check 'active_endpoint_with_link_down_sends_exact_reconnect' ($r.Accepted -and $r.Requested -eq 1 -and $f.Calls[0] -eq 'render:0')
$f=Fake @((Ep 'render' 0 1));$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$false,-1)
Check 'unknown_link_fails_before_active_reconnect' (!$r.Accepted -and $r.Requested -eq 0 -and $f.Calls.Count -eq 0)
$f=Fake @((Ep 'render' 0 8));$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$false,-1)
Check 'unknown_link_fails_before_unplugged_reconnect' (!$r.Accepted -and $r.Requested -eq 0 -and $f.Calls.Count -eq 0)
Check 'invalid_link_address_read_returns_unknown' ([AirPodsBuddy.Ks.BluetoothLink]::Read('not_an_address') -eq -1)
...
$f=Fake @((Ep 'render' 0 1),(Ep 'capture' 1 1),(Ep 'other' 1 1 $b));$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$false,$false,0)
...
$f=Fake @((Ep 'render' 0 8),(Ep 'capture' 1 8));$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$false,$false,0)
...
$f=Fake @((Ep 'render' 0 1 $a 'shared'),(Ep 'capture' 1 1 $a 'shared'));$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$false,$false,0)
Check 'shared_ks_filter_requested_only_once' ($r.Accepted -and $f.Calls.Count -eq 1)
$f=Fake @((Ep 'render' 0 1),(Ep 'capture' 1 1));$f.Rows[1].DisconnectSupported=$false;$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$false,$false,0)
Check 'unsupported_capture_aborts_before_any_disconnect' (!$r.Accepted -and $f.Calls.Count -eq 0)
$f=Fake @((Ep 'render' 0 8));$f.Rows[0].ReconnectSupported=$false;$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$false,0)
Check 'unsupported_connect_does_not_toggle_services' (!$r.Accepted -and $f.Calls.Count -eq 0)
$f=Fake @((Ep 'render' 0 2));Check 'user_disabled_endpoint_not_enabled' (MustThrow {[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$false,0)})
$f=Fake @((Ep 'render' 0 4));Check 'notpresent_endpoint_not_connected' (MustThrow {[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$false,0)})
$f=Fake @((Ep 'r1' 0 8),(Ep 'r2' 0 8));Check 'ambiguous_stereo_endpoints_fail_closed' (MustThrow {[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$false,0)})
$f=Fake @((Ep 'r' 0 8),(Ep 'hfp' 0 8 $a 'BTHHFENUM'));$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$false,0)
Check 'known_hfp_render_not_selected_as_stereo' ($r.Accepted -and $r.RenderId -eq 'r' -and $f.Calls.Count -eq 1)
$f=Fake @((Ep 'r' 0 8),(Ep 'capture' 1 8));$f.FailAt=1;$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$true,0)
...
$f=Fake @((Ep 'r' 0 8));$f.ThrowAt=1;$r=[AirPodsBuddy.Ks.Policy]::Run($f,$a,$true,$false,0)
...
Check 'empty_container_rejected' (MustThrow {[AirPodsBuddy.Ks.Policy]::Run($f,[guid]::Empty.ToString(),$true,$false,0)})
